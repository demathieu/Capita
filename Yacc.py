...
def p_statement_exp_statement_while(p):
    'statement  : WHILE LPAR exp statement'
    ...
    for item in code:
        ...
        if item.op and item.arg1 and item.arg2 and item.result:
            return_string = return_string + "\t" +  result + " = " + arg1 + " " + op + " " + arg2 + ";\n"
                ...
                #if not item.arg1:
                    return_string = return_string + "\t" + "BeginFunc" + ' '+ str(item.arg1) + ";\n"
        elif item.op == "IfNode":
...
def GenerateTAC(data):
    #print "start parsing \n"
    yacc.parse(data)
    #printQuadruplesToTac(listTAC)
    #show_TAC(listTAC)
    for i, codeLine in enumerate(listTAC):
        if codeLine.op=='TO_CHECK_FunctionSize':
            listTAC[i]= Quadruple('BeginFunc',scope.giveBackSizeForFunction(codeLine.arg2),None,None)
        elif codeLine.arg1 == 'TO_CHECK_FormalParamsSize':
            listTAC[i]= Quadruple('PopParams',scope.giveBackSizeForFormalParam(codeLine.arg2),None,None)
